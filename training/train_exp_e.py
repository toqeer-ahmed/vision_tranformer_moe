    ...
    for stage_idx, stage in enumerate(segformer.encoder.block):
        ...
        for block_idx, block in enumerate(stage):
            ...
            if use_shared_expert:
                experts_to_init.append(moe_layer.shared_expert)
                experts_to_init.extend(list(moe_layer.routed_moe.experts))
            else:
                experts_to_init.extend(list(moe_layer.experts))
                
            with torch.no_grad():
                for expert in experts_to_init:
                    # dense1 (Linear)
                    expert.dense1.weight.copy_(old_mlp.dense1.weight)
                    expert.dense1.bias.copy_(old_mlp.dense1.bias)
                    
                    # dwconv (Conv2d)
                    # Note: old_mlp.dwconv is a SegformerDWConv which has .dwconv
                    expert.dwconv.weight.copy_(old_mlp.dwconv.dwconv.weight)
                    expert.dwconv.bias.copy_(old_mlp.dwconv.dwconv.bias)
                    
                    # dense2 (Linear)
                    expert.dense2.weight.copy_(old_mlp.dense2.weight)
                    expert.dense2.bias.copy_(old_mlp.dense2.bias)
...
def train(config_path: str):
    ...
    # Setup logger
    logger = setup_logger("train_exp_e", log_dir=log_cfg["log_dir"])
    logger.info(f"Starting Experiment E (Warm Init Spatially-Aware MoE) training for {model_cfg['name']} on {dataset_cfg['name']} dataset.")
    ...
    # Dynamic swap to MoE Architecture
    logger.info("Integrating Mixture of Experts (MoE) routing...")
    ...
    for epoch in range(1, epochs_to_run + 1):
        ...
        # EXPERIMENT E DIAGNOSTICS: Print Expert Utilization
        total_counts = None
        for name, module in model.named_modules():
            if hasattr(module, "expert_token_counts"):
                counts = module.expert_token_counts.detach().cpu()
        ...
        if total_counts is not None:
            ...
            if total > 0:
                ...
                logger.info(f"Epoch {epoch} Expert Utilization: " + " | ".join([f"E{i}: {p:.1f}%" for i, p in enumerate(percentages)]))
