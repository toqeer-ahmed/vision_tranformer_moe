...
| Model | Parameters | mIoU | mDice |
| :--- | :--- | :--- | :--- |
| **SegMoTE (SAM Foundation Model)** | 94.20 Million | 65.99% | 74.07% |
| **MoE-Segformer (Proposed Approach)**| **8.07 Million** | **65.12%** | **75.23%** |

### Key Findings
1. **Comparable mIoU with 11x Fewer Parameters:** The proposed MoE-Segformer achieved a 65.12% mIoU, nearly matching the massive 94 Million parameter SegMoTE foundation model (65.99%).
2. **Superior Dice Score:** Despite its diminutive size, the MoE-Segformer outperformed SegMoTE on the mean Dice Score (75.23% vs. 74.07%), demonstrating that end-to-end domain-specific training on medical imagery yields higher overlap accuracy than relying on frozen natural-image encoders.
3. **Training Efficiency:** The MoE-Segformer trains significantly faster and requires vastly lower VRAM, making it a highly practical solution for real-time clinical deployment compared to SAM-based architectures.
